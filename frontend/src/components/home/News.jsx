  ...
  const newsArticles = [
    {
      ...
      description:
        "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed vel turpis vel neque rutrum facilisis. Nulla facilisi.",
    ...
    },
    {
      ...
      description:
        "Donec varius, orci at ultrices consectetur, mauris neque accumsan diam, id elementum neque ipsum eu turpis.",
      image: "https://via.placeholder.com/300",
      views: 45,
      likes: 72,
    },
        ...
        {[...Array(Math.ceil(newsArticles.length / 4))].map((_, slideIndex) => (
          ...
          >
            <div className="grid grid-cols-2 grid-rows-2 gap-3">
              {newsArticles
                .slice(slideIndex * 4, slideIndex * 2 + 4)
                .map((article, index) => (
                  <div
                    key={index}
                    className="bg-white rounded-lg shadow-md p-4  text-center flex gap-2 w-full"
                  >
                    <img
                      src={article.image}
                      alt={article.title}
                      className="w-32 h-full rounded-md  mb-4"
                    />
